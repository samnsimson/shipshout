...
export enum Channel { X = 'x', LinkedIn = 'linkedin', Email = 'email', Buffer = 'buffer', Mailchimp = 'mailchimp' }
export enum DraftStatus { PendingReview = 'pending_review', Approved = 'approved', Published = 'published', Failed = 'failed' }
...
export function buildPrompt(input: {
  commitSummary: string; tone: Tone; customInstructions?: string; emojiPolicy: boolean; channel: Channel;
...
const bad = (name: string) => ({ name, generate: jest.fn(async () => { throw new Error('boom'); }) });
...
export interface AiResult { text: string; model: string; tokens?: number; }
...
export class OpenAiProvider implements AiProvider {
  ...
  async generate(prompt: AiPrompt, opts?: { maxTokens?: number }): Promise<AiResult> {
    ...
    const res = await this.client.chat.completions.create({
      model, max_tokens: opts?.maxTokens ?? 400,
...
export interface EngineResult { text: string; provider: string; model: string; tokens?: number; latencyMs: number; }
...
function deps() {
  const event = { id:'e1', commitSummary:'fix cache latency', status:'received',
...
export class GenerationService {
  ...
  async generateForEvent(releaseEventId: string, channels: Channel[]): Promise<Draft[]> {
    ...
    const brand = (await this.brands.findOne({ where: { workspace: { id: workspaceId } } }))
    ...
    for (const channel of channels) {
      const prompt = buildPrompt({
        ...
        tone: brand.tone, customInstructions: brand.customInstructions, emojiPolicy: brand.emojiPolicy, channel,
      });
      ...
      const draft = await this.drafts.save(this.drafts.create({
        releaseEvent: event as any, channel, generatedCopy: text, status: DraftStatus.PendingReview,
...
export class GenerateProcessor extends WorkerHost {
  constructor(private generation: GenerationService) { super(); }
