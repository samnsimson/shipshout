...
describe('DispatchService.dispatch', () => {
  it('publishes and records success', async () => {
    ...
    const svc = new DispatchService(
      ...
      d.records as any,
      d.registry as any,
      d.connections as any,
    );
    await svc.dispatch('d1');
    expect(d.records.save).toHaveBeenCalledWith(
      expect.objectContaining({ status: PublishStatus.Success, externalUrl: 'https://x.com/1' }),
    );
  ...
  it('records failure and marks draft failed', async () => {
    ...
    expect(d.records.save).toHaveBeenCalledWith(
      expect.objectContaining({ status: PublishStatus.Failed }),
    );
  });
});
